...
MarkdownTextEdit::MarkdownTextEdit(QWidget* parent) {
   // highlighter = new MarkdownHighlighter(document());
    ...
    InsertCodeShortcut = new QShortcut(QKeySequence("Ctrl+Shift+C"), this);
    ...
    connect(InsertCodeShortcut, &QShortcut::activated, this, &MarkdownTextEdit::InsertCode);
...
void MarkdownTextEdit::InsertCode(){

    QTextCursor cursor = textCursor();

    if(cursor.currentFrame() != document()->rootFrame()){
        if (cursor.currentFrame()->format().isTableCellFormat())
            return;
    }
